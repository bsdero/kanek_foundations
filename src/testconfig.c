...
#define CHECK_STR(v, expected, desc) do {                              \
    const char *_s = ((v) != NULL) ? var_to_str( &gc, (v)) : NULL;      \
    CHECK( _s != NULL && strcmp( _s, (expected)) == 0, desc);            \
} while ( 0)

#define CHECK_INT(v, expected, desc) \
    CHECK( (v) != NULL && var_to_int( v) == (int64_t)(expected), desc)

#define CHECK_BOOL(v, expected, desc) \
    CHECK( (v) != NULL && var_to_bool( v) == (expected), desc)
...
int main( void){
    ...
    CHECK( v != NULL && fabs( var_to_float( v) - 3.14) < 1e-9,
          "MYFLOAT ~= 3.14");
    ...
    CHECK( v != NULL && v->type == VAR_ARRAY, "MYARRAY type == VAR_ARRAY");
    CHECK( v != NULL && var_len( v) == 6, "MYARRAY length == 6");
    ...
    CHECK( v != NULL && fabs( var_to_float( v) - 2.71828) < 1e-4,
          "INCLUDED_FLOAT ~= 2.71828");
