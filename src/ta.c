...
void *ta_malloc( ta_list_t *ll, size_t size){
    ...
    if ( node == NULL) {
        TRACE_ERR( "malloc failed for %zu bytes", total);
        return( NULL);
    }
...
void *ta_realloc( ta_list_t *ll, void *ptr, size_t size){
    ...
    if ( mem == NULL) {
        TRACE_ERR( "realloc failed for %zu bytes", size);
...
char *ta_strdup( ta_list_t *ll, char *p){
    ...
    if ( m == NULL) {
        TRACE_ERR( "ta_malloc failed for %zu bytes", string_size + 1);
        return( NULL);
    }
...
char *ta_strncat( ta_list_t *ll, char *p, char *q){
    ...
    if ( pstr == NULL) {
        TRACE_ERR( "ta_realloc failed for %zu bytes", string_size);
        return( NULL);
    }
...
void *ta_memclone( ta_list_t *ll, void *p, int n){
    ...
    if ( pm == NULL) {
        TRACE_ERR( "ta_malloc failed for %d bytes", n);
        return( NULL);
    }
    memcpy( pm, p, n);
    return( pm);
