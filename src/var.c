...
var_t *var_int( ta_list_t *gc, int64_t v){
    var_t *var = ta_malloc( gc, sizeof( var_t));
    if ( var == NULL) {
...
var_t *var_str( ta_list_t *gc, const char *s){
    ...
    var->type = VAR_STR;
    var->s = (s != NULL) ? ta_strdup( gc, (char *)s) : NULL;
    return( var);
}
...
var_t *var_array( ta_list_t *gc){
    ...
    var->type = VAR_ARRAY;
    var->arr.items = ta_malloc( gc, ARRAY_INIT_CAP * sizeof( var_t *));
    if ( var->arr.items == NULL) {
        TRACE_ERR( "ta_malloc failed for array items");
        ta_free( var);
        return( NULL);
    }
    var->arr.len = 0;
...
int var_push( ta_list_t *gc, var_t *arr, var_t *item){
    if ( arr == NULL || arr->type != VAR_ARRAY) {
        TRACE_ERR( "arr is NULL or not a VAR_ARRAY");
        return( -1);
    }

    if ( arr->arr.len == arr->arr.cap) {
        size_t new_cap = arr->arr.cap * 2;
        var_t **grown = ta_realloc( gc, arr->arr.items,
                                   new_cap * sizeof( var_t *));
        if ( grown == NULL) {
            TRACE_ERR( "ta_realloc failed for array items");
            return( -1);
        }
        arr->arr.items = grown;
        arr->arr.cap   = new_cap;
...
int64_t var_to_int( var_t *v){
    ...
    switch ( v->type) {
        ...
        case VAR_STR:
            return( (v->s != NULL) ? strtoll( v->s, NULL, 10) : 0);
        default:        return( 0);
    }
}
...
double var_to_float( var_t *v){
    ...
    switch ( v->type) {
        ...
        case VAR_STR:   return( (v->s != NULL) ? strtod( v->s, NULL) : 0.0);
        default:        return( 0.0);
    }
}
...
int var_to_bool( var_t *v){
    ...
    switch ( v->type) {
        ...
        case VAR_STR:   return( (v->s != NULL) && v->s[0] != '\0');
...
char *var_to_str( ta_list_t *gc, var_t *v){
    ...
    if ( v == NULL) {
        return( ta_strdup( gc, "null"));
    }
    switch ( v->type) {
        case VAR_INT:
            snprintf( buf, sizeof( buf), "%" PRId64, v->i);
            return( ta_strdup( gc, buf));
        case VAR_FLOAT:
            snprintf( buf, sizeof( buf), "%g", v->f);
            return( ta_strdup( gc, buf));
        case VAR_BOOL:
            return( ta_strdup( gc, v->b ? "true" : "false"));
        case VAR_STR:
            return( ta_strdup( gc, (v->s != NULL) ? v->s : "null"));
        case VAR_NULL:
            return( ta_strdup( gc, "null"));
        case VAR_ARRAY:
            return( ta_strdup( gc, "[array]"));
        case VAR_DICT:
            return( ta_strdup( gc, "{dict}"));
        default:
            return( ta_strdup( gc, "null"));
    }
}
