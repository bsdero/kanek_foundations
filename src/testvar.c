...
static void test_primitives( ta_list_t *gc){
    ...
    CHECK( vs->type == VAR_STR  && strcmp( vs->s, "hello") == 0,
          "str value");
...
static void test_coercions( ta_list_t *gc){
    ...
    char *s;
    s = var_to_str( gc, vi);
    CHECK( strcmp( s, "7") == 0,     "int->str");
    s = var_to_str( gc, vb);
    CHECK( strcmp( s, "true") == 0,  "bool->str true");
    s = var_to_str( gc, var_bool( gc, 0));
    CHECK( strcmp( s, "false") == 0, "bool->str false");
    s = var_to_str( gc, vs);
    CHECK( strcmp( s, "99") == 0,    "str->str");
    s = var_to_str( gc, vn);
    CHECK( strcmp( s, "null") == 0,  "null->str");
}
...
static void test_dict( ta_list_t *gc){
    ...
    CHECK( v != NULL && strcmp( v->s, "kanek") == 0, "get correct value");
...
static void test_nested( ta_list_t *gc){
    ...
    CHECK( got != NULL && got->type == VAR_ARRAY, "array value in dict");
    ...
    CHECK( got_meta != NULL && got_meta->type == VAR_DICT,
          "dict value in dict");
    dict_t *meta_inner = (dict_t *)got_meta->dict;
    CHECK( strcmp( var_to_str( gc, dict_get( meta_inner, "author")),
                 "armando") == 0,
          "deeply nested value");
}
