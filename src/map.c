...
int byte_count_bits( int start, int numbits, unsigned char byte,
                     int clear_or_set){
    int to = start + numbits; /* which bit to stop */
    uint8_t mask = 0x00;
    int count = 0;
...
int bm_set_bit( unsigned char *bm,
    ...
    if ( bit_address >= total_bits) {
        TRACE_ERR( "bit_address %lu out of range (total_bits %lu)",
                  (unsigned long)bit_address, (unsigned long)total_bits);
        return( -1);
    }
...
int bm_count( unsigned char *bm,
    ...
    *count_result = 0;
    ...
    if ( (bit_address >= total_bits) ||
        ...
        TRACE_ERR( "bit_address/num_bits_to_count out of range");
        return( -1);
    }
...
int bm_find( unsigned char *bm,
    ...
    if ( (bit_address >= total_bits) ||
        ...
        (count == 0)                    ||
        (gap_size > count)) {
        TRACE_ERR( "invalid bm_find arguments");
        *found_address = 0;
        return( -1);
    }
    ...
    do {
        ...
        if ( rc >= 0) {
            ...
            /* If the remaining bits needed (updated_gap_size) exceed
             * the remaining search range (count), the gap cannot
             * be completed — stop now. */
            if ( count == 0 || updated_gap_size > count) {
                *found_address = 0;
                return( -1);
            }
        }
...
int bm_extent_can_grow( unsigned char *bm,
    ...
    if ( (bit_address >= total_bits)         ||
        ...
        TRACE_ERR( "invalid bm_extent_can_grow arguments");
        return( -1);
    }
...
/* Clear all bits — mark the entire bitmap as free. */
void bm_zero( unsigned char *bm, uint64_t total_bits){
    uint64_t bytes = (total_bits + 7) / 8;
    memset( bm, 0x00, bytes);
}

/* Set all bits — mark the entire bitmap as used. */
void bm_fill( unsigned char *bm, uint64_t total_bits){
    uint64_t bytes     = total_bits / 8;
    uint64_t tail_bits = total_bits % 8;

    memset( bm, 0xff, bytes);

    /* Partial last byte: only set the bits that are within total_bits. */
    if ( tail_bits > 0) {
        bm[bytes] = (unsigned char)((1u << tail_bits) - 1u);
    }
}

/* Count the total number of set (1) bits in the bitmap. */
uint64_t bm_popcount( unsigned char *bm, uint64_t total_bits){
    ...
    for ( i = 0; i < bytes; i++) {
#ifdef __GNUC__
        count += (uint64_t)__builtin_popcount( bm[i]);
#else
    ...
    /* Partial last byte — count only bits within total_bits. */
    if ( tail_bits > 0) {
        unsigned char last = bm[bytes] &
                              (unsigned char)((1u << tail_bits) - 1u);
#ifdef __GNUC__
        count += (uint64_t)__builtin_popcount( last);
#else
        while ( last) {
            count += last & 1u;
            last >>= 1;
